...
// Email configuration
const EMAIL_HOST = env.email.host;
const EMAIL_PORT = env.email.port;
const EMAIL_USER = env.email.user;
const EMAIL_PASS = env.email.pass;
const EMAIL_FROM = env.email.from;
const CLIENT_URL = env.app.clientUrl;

// Create transporter
const createTransporter = (): nodemailer.Transporter => {
  // Use test transporter for testing environment
  if (env.isTest) {
...
// Email templates
const getEmailTemplate = (type: 'verification' | 'password-reset' | 'download-ready' | 'payment-receipt' | 'subscription-expiry', data: any) => {
  const baseStyles = `
    <style>
...
// Send subscription confirmation email (for future use with Stripe)
...
// Send download ready notification email
