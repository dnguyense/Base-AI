...
function activate(context) {
  console.log(
    "Appdexer V2 extension is now active in Cursor for MCP integration!"
  );

  // Create output channel for logging
  outputChannel = vscode.window.createOutputChannel("Appdexer V2");
  context.subscriptions.push(outputChannel);

  // Silent activation - only log to console, not output channel
  console.log(
    "Appdexer V2 extension activated for Cursor MCP integration by Kiên Lê & Lakshman Turlapati"
  );

  // Register command to open Appdexer manually
  let disposable = vscode.commands.registerCommand("appdexer.openChat", () => {
    openReviewGatePopup(context, {
      message: "Welcome to Appdexer! Please provide your review or feedback.",
      title: "Appdexer",
    });
  });
  ...
  // Start Appdexerintegration immediately
  startReviewGateIntegration(context);

  // Show activation notification
  vscode.window.showInformationMessage(
    "Appdexer activated! Use Cmd+Shift+A or wait for MCP tool calls."
  );
}
...
function logUserInput(
  ...
  console.log(`APPDEXER USER INPUT: ${inputText}`);
  ...
  try {
    ...
    logMessage(`Could not write to Appdexerlog file: ${error.message}`);
  }
}
...
function checkMcpStatus() {
  try {
    ...
    const mcpLogPath = getTempPath("appdexer.log");
...
function startReviewGateIntegration(context) {
  ...
  // Watch for Appdexertrigger file
  ...
  vscode.window.showInformationMessage(
    "AppdexerV2 MCP integration ready! Extension is monitoring for Cursor Agent tool calls..."
  );
}
...
function checkTriggerFile(context, filePath) {
  try {
    if (fs.existsSync(filePath)) {
      ...
      if (triggerData.system && triggerData.system !== "appdexer") {
        return;
      }

      // Only log essential trigger info
      console.log(`Appdexertriggered: ${triggerData.data.tool}`);
...
function handleReviewGateToolCall(context, toolData) {
  ...
  switch (toolData.tool) {
    case "review_gate":
      ...
      let modeTitle = `Appdexer V2 - ${
        mode.charAt(0).toUpperCase() + mode.slice(1)
      } Mode`;
      if (toolData.unified_tool) {
        modeTitle = `Appdexer V2 ゲート - Unified (${mode})`;
      }
    ...
    case "review_gate_chat":
      popupOptions = {
        ...
        title: toolData.title || "Appdexer V2 - ゲート",
        autoFocus: true,
        toolData: toolData,
        mcpIntegration: true,
      };
    ...
    case "quick_review":
      popupOptions = {
        ...
        title: toolData.title || "Appdexer V2 ゲート - Quick Review",
        autoFocus: true,
        toolData: toolData,
        mcpIntegration: true,
    ...
    case "ingest_text":
      popupOptions = {
        message: `Cursor Agent received text input and needs your feedback:\n\n**Text Content:** ${
          toolData.text_content
        }\n**Source:** ${toolData.source}\n**Context:** ${
          toolData.context || "None"
        }\n**Processing Mode:** ${
          toolData.processing_mode
        }\n\nPlease review and provide your feedback:`,
        title: toolData.title || "Appdexer V2 ゲート - Text Input",
        autoFocus: true,
        toolData: toolData,
        mcpIntegration: true,
      };
    ...
    case "shutdown_mcp":
      popupOptions = {
        message: `Cursor Agent is requesting to shutdown the MCP server:\n\n**Reason:** ${
          ...
          toolData.cleanup ? "Yes" : "No"
        }\n\nType 'CONFIRM' to proceed with shutdown, or provide alternative instructions:`,
        title: toolData.title || "Appdexer V2 ゲート - Shutdown Confirmation",
        autoFocus: true,
        toolData: toolData,
        mcpIntegration: true,
    ...
    case "file_review":
      popupOptions = {
        ...
        title: toolData.title || "Appdexer V2 ゲート - File Review",
        autoFocus: true,
        toolData: toolData,
        mcpIntegration: true,
      };
    ...
    default:
      popupOptions = {
        ...
        title: toolData.title || "Appdexer V2 ゲート - General Input",
        autoFocus: true,
        toolData: toolData,
        mcpIntegration: true,
      };
  ...
  // Immediately open Appdexer popup when tools are triggered by Cursor Agent
  ...
  vscode.window.showInformationMessage(
    `Cursor Agent triggered "${toolDisplayName}" - Appdexer popup opened for your input!`
  );
}
...
function openReviewGatePopup(context, options = {}) {
  const {
    message = "Welcome to Appdexer V2! Please provide your review or feedback.",
    title = "Review Gate",
    autoFocus = false,
    toolData = null,
...
<body>
    <div class="review-container">
        <div class="review-header">
            ...
            <div class="review-author">by Kiên Lê</div>
        </div>
...
function handleReviewMessage(
  ...
  const funnyResponses = [
    "Đã gửi đánh giá - Giữ chặt quần của bạn cho đến khi Appdexer được gọi lại! 🎢",
    "Đã gửi tin nhắn! Có lẽ Agent đang làm những việc của Agent ngay bây giờ... ⚡",
    "Trí tuệ của bạn đã được truyền đến các lãnh chúa kỹ thuật số! 🤖",
    "Phản hồi được đưa vào hư không - mong đợi phép thuật của Agent sớm! ✨",
    "Appdexer đã đóng - Agent đang nhai thông tin đầu vào của bạn! 🍕",
    "Đã nhận tin nhắn và lưu vào mục 'Có thể quan trọng'! 📁",
    "Thông tin đầu vào của bạn hiện là một phần trong kế hoạch tổng thể của Agent! 🧠",
    "Đã gửi đánh giá - Agent nợ bạn một lần! 🤝",
    "Thành công! Suy nghĩ của bạn hiện đang ám ảnh giấc mơ của Agent! 👻",
    "Đã giao nhanh hơn cả pizza vào tối thứ sáu! 🍕",
  ];
...
async function handleSpeechToText(audioData, triggerId, isFilePath = false) {
  try {
    ...
    if (isFilePath) {
      ...
      console.log(
        `Using existing audio file for transcription: ${tempAudioPath}`
      );
    } else {
      ...
      tempAudioPath = getTempPath(
        `review_gate_audio_${triggerId}_${Date.now()}.wav`
      );
      ...
      if (require("fs").existsSync(resultFile)) {
        try {
          const result = JSON.parse(
            require("fs").readFileSync(resultFile, "utf8")
          );
...
function startNodeRecording(triggerId) {
  try {
    ...
    const audioFile = getTempPath(
      `review_gate_audio_${triggerId}_${timestamp}.wav`
    );
    ...
    console.log(
      `✅ SoX recording started: PID ${currentRecording.pid}, file: ${audioFile}`
    );
...
function stopNodeRecording(triggerId) {
  try {
    ...
    console.log(
      `🛑 Stopping SoX recording: PID ${recordingPid}, file: ${audioFile}`
    );
    ...
    currentRecording.on("exit", (code, signal) => {
      ...
      setTimeout(() => {
        ...
        if (fs.existsSync(audioFile)) {
          ...
          console.log(
            `✅ Audio file created: ${audioFile} (${stats.size} bytes)`
          );
